...
public class Board {
	...
	/**
	 * hugged determines if the game ends or not based on if jarvis has
	 * been hugged or not. It is a boolean and is set to true when he is
	 * finally hugged
	 */
	...
	public synchronized boolean move(Direction dir, Boardable elem) {
		...
		if(dir.toString().equalsIgnoreCase("UP")) {//the equalsIgnoreCase is overkill but checking all the boxes
			...
			ret = makeChanges(elem, 1, 1);
		}else if(dir.toString().equalsIgnoreCase("STAY")) {
	...
	/**
	 * This method returns the row that the passed element is in
	 * @param elem - the element which row is wanted
	 * @return - an int indicating the row the element is in
	 */
	public int getRow(Boardable elem) {
		return elementPlace.get(elem).getRow();
	}
	/**
	 * This method returns the column that the passed element is in
	 * @param elem - the element which column is wanted
	 * @return - an int indicating the column the element is in
	 */
	...
	/**
	 * This method removes the element passed from the cell it is in
	 * @param elem - element that is needed to be removed
	 * @return - returns a true or false based on if it was able to remove the element or not
	 */
	public boolean removeElement(Boardable elem) {
		Cell temp = elementPlace.get(elem);
		return temp.removeElement(elem);
	}
	/**
	...
	/**
	 * This method sets this.hugged to the passed in hugged variable
	 * @param hugged - is what is used to set the this.hugged
	 */
	public synchronized void setHugged(boolean hugged) {
		this.hugged = hugged;
	}
	/**
	 * This method tells us if jarvis has been hugged or not
	 * @return - returns if he has been hugged or not
	 */
	...
	private class Cell{
		 ...
		 * it only adds the element if the cell is empty or the other 
		 * item in the cell can share with the new element
		...
		public String toString() {
			...
			}else if((!elements.isEmpty() && elements.get(0).isVisible() == false)) {
				ret = elements.get(elements.size()-1).toString();// if the visibilty is true and there is no element inside or if the elements visibility is false and the 
